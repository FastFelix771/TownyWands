			...
			if (isCommandHandler(method)) {
				...
				if (plugin.getCommand(annotation.name()) != null) {
					...
					if (!(annotation.aliases().equals(new String[] { "" })))
						plugin.getCommand(annotation.name()).setAliases(Arrays.asList(annotation.aliases()));
					...
					if (!annotation.permissionMessage().equals(""))
						plugin.getCommand(annotation.name()).setPermissionMessage(annotation.permissionMessage());
